    ...
    var body: some View {
        ZStack {
            VStack {
                RoundedRectangle(cornerRadius: 25)
                    ...
                    .onTapGesture {
                        if let url = openAppSelectorPanel() {
                            restoreAndPatch(url: url, opts: &opts, onPatch: { progressVisible = true })
                        }
                    }
                    .onDrop(of: [.fileURL], delegate: FileDropDelegate(opts: $opts, onPatch: { progressVisible = true }))
                //        if(externalUrl != nil) {
                ...
                Text(String(opts.getTotalProgress()))
                Text(String(opts.progress))
            }
            ProgressDialog(opts: $opts, visible: $progressVisible)
        }
    }
}
